...
describe('NounsDescriptor', () => {
  ...
  it('should generate valid token uri metadata when data uris are enabled', async () => {
    ...
    expect(name).to.equal('Social Noun 0');
    expect(description).to.equal('Social Noun 0 is a member of the Social Nouns DAO');
  ...
  it.skip('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      ...
      expect(name).to.equal(`Social Noun ${i}`);
      expect(description).to.equal(`Social Noun ${i} is a member of the Social Nouns DAO`);
      expect(image).to.not.be.undefined;
