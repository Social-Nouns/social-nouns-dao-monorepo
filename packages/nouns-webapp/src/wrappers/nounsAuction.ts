...
const auctionhousecontract = new NounsAuctionReferralFactory().attach(config.addresses.nounsAuctionReferral);

export const useCreateBidReferral = () => {
  const { send: createBidReferral, state: createBidReferralState } = useContractFunction(
    auctionhousecontract,
    'createBid',
  );
  return { createBidReferral, createBidReferralState };
};
